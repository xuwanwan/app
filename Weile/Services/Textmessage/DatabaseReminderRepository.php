...
class DatabaseReminderRepository implements ReminderRepositoryInterface {
	 ...
	 * @return array
	 */
	public function getPayload($phone, $token)
	{
	...
	public function exists(RemindableInterface $user, $token)
	{
		...
		$reminder = (array) $this->getTable()->where('phone', $phone)->where('token', $token)->first();
	...
	{
		$this->getTable()->where('phone', $token)->delete();
	}
