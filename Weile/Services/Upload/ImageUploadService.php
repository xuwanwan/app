...
namespace Weile\Services\Upload;
...
class ImageUploadService
{
     ...
     * @var string
     */
    protected $directory = 'img/avatar';
    ...
    protected function getRelativePath() {
    	return $this->directory. '/'.$this->getDatePath();
    }
     ...
     * @return array
     */
    protected function getJsonBody($filename, $mime, $path, $relativefile)
    {
        return [
            'images' => [
                'filename' => $filename,
    ...
    public function handle(UploadedFile $file)
    {
        ...
        $this->makeDir($this->getFullPath($relativepath));

        $relativefile = $relativepath.'/'. $filename;

        $path     = $this->getFullPath($relativefile);
        ...
        return $this->getJsonBody($filename, $mime, $path, $relativefile);
    }
}
